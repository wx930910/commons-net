...
public class UtilTest {
	...
	public static CopyStreamListener mockCopyStreamListener1(long totalBytesTransferred, int bytesTransferred,
			long streamSize) {
		long mockFieldVariableExpectedTotal;
		int mockFieldVariableExpectedBytes;
		long mockFieldVariableExpectedSize;
		CopyStreamListener mockInstance = mock(CopyStreamListener.class);
		mockFieldVariableExpectedTotal = totalBytesTransferred;
		mockFieldVariableExpectedBytes = bytesTransferred;
		mockFieldVariableExpectedSize = streamSize;
		doAnswer((stubInvo) -> {
			long totalBytesTransferredMockVariable = stubInvo.getArgument(0);
			int bytesTransferredMockVariable = stubInvo.getArgument(1);
			long streamSizeMockVariable = stubInvo.getArgument(2);
			Assert.assertEquals("Wrong total", mockFieldVariableExpectedTotal, totalBytesTransferredMockVariable);
			Assert.assertEquals("Wrong streamSize", mockFieldVariableExpectedSize, streamSizeMockVariable);
			Assert.assertEquals("Wrong bytes", mockFieldVariableExpectedBytes, bytesTransferredMockVariable);
			return null;
		}).when(mockInstance).bytesTransferred(anyLong(), anyInt(), anyLong());
		return mockInstance;
	}
	...
	public void testReader0() throws Exception {
		long streamSize = 0;
		int bufferSize = 0;
		Util.copyReader(source, dest, bufferSize, streamSize, UtilTest.mockCopyStreamListener1(1, 1, streamSize));
	}
	...
	public void testStream0() throws Exception {
		long streamSize = 0;
		int bufferSize = 0;
		Util.copyStream(src, dst, bufferSize, streamSize, UtilTest.mockCopyStreamListener1(1, 1, streamSize));
	}
